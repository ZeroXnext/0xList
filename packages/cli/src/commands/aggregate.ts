...
export default function addAggregate(entry: Entry, config: Config) {
  entry.command(
    "aggregate",
    "It aggregates a directory of token lists into <directory>/index.json",
    () => {},
    () => {
      ...
      forEachIndex((_, indexPath) => {
        ...
        const listIndex = buildIndex(
          path.join(config.contentBaseURL, username, repo),
          path.join(config.outputDir, indexPath),
          loaded,
        );
        indexes.push(listIndex);
      }, config);

      const chainTypeIndexes = mergeIndexes(
        indexes,
        path.join(config.contentBaseURL, username, repo, config.outputDir),
      );
      for (const [networkType, listIndex] of chainTypeIndexes.entries()) {
        fs.writeFileSync(
          path.join(config.outputDir, networkType, `${config.indexFileName}.json`),
          JSON.stringify(listIndex, null, 2),
          "utf8",
        );
      }
    },
