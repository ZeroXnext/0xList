...
function main(outputDir: string): [TokenListsMap, Set<string>, string[]] {
  ...
  for (const [chainType, chainName, listKey] of lists) {
    ...
    const currentPath = path.join(outputDir, chainType, chainName, listKey);
    paths.push(currentPath);
    const tokenList = JSON.parse(fs.readFileSync(currentPath, 'utf-8')) as TokenList;
